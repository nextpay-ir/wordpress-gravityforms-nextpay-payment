...
class GFNextPayData
{
    ...
    public static function update_table()
    {
        ...
        $table_name = self::get_nextpay_table_name();
    ...
    public static function get_nextpay_table_name()
    {
        global $wpdb;
        return $wpdb->prefix . "rg_nextpay";
    }

    public static function drop_tables()
    {
        global $wpdb;
        $wpdb->query("DROP TABLE IF EXISTS " . self::get_nextpay_table_name());
    }
    ...
    //----جمع پرداخت های نکست پی این فرم----------
    public static function get_transaction_totals($form_id)
    {
        ...
        $sql = $wpdb->prepare(" SELECT l.status, sum(l.payment_amount) revenue, count(l.id) transactions
                                 ...
                                 GROUP BY l.status", $form_id, 'active', 1, 'nextpay');
        $results = $wpdb->get_results($sql, ARRAY_A);
        $totals = array();
        if (is_array($results)) {
    ...
    //-----جمع پرداخت های  همه فرمهای نکست پی-----------
    public static function get_transaction_totals_nextpay()
    {
        global $wpdb;
        $lead_table_name = RGFormsModel::get_lead_table_name();
        $sql = $wpdb->prepare(" SELECT l.status, sum(l.payment_amount) revenue, count(l.id) transactions
                                 ...
                                 GROUP BY l.status", 'active', 1, 'nextpay');
        $results = $wpdb->get_results($sql, ARRAY_A);
        $totals = array();
        if (is_array($results)) {
            foreach ($results as $result) {
