...
register_activation_hook(__FILE__, array("GFNextPay_nextpayStd", "add_permissions"));
add_action('init', array('GFNextPay_nextpayStd', 'init'));

require_once('functions.php');
require_once('database.php');
require_once('chart.php');

class GFNextPay_nextpayStd
{

    //Dont Change this Parameter if you are legitimate !!!
    public static $author = "nextpayStd";
    ...
    public static function init()
    {
		if (!class_exists("GFParsi")) {
			add_action('admin_notices', array('GFNextPay_nextpayStd', 'admin_notice_persian_gf'));
			return false;
		}

		if (!self::is_gravityforms_supported()) {
			add_action('admin_notices', array('GFNextPay_nextpayStd', 'admin_notice_gf_support'));
			return false;
		}
        ...
        if (is_admin() && self::has_access()) {
            ...
            if (function_exists('members_get_capabilities'))
                add_filter('members_get_capabilities', array("GFNextPay_nextpayStd", "members_get_capabilities"));

            add_filter('gform_tooltips', array('GFNextPay_nextpayStd', 'tooltips'));
            add_filter('gform_addon_navigation', array('GFNextPay_nextpayStd', 'menu'));
            add_action('gform_entry_info', array('GFNextPay_nextpayStd', 'payment_entry_detail'), 4, 2);
            add_action('gform_after_update_entry', array('GFNextPay_nextpayStd', 'update_payment_entry'), 4, 2);

            if (get_option("gf_nextpay_configured")) {
                add_filter('gform_form_settings_menu', array('GFNextPay_nextpayStd', 'toolbar'), 10, 2);
                add_action('gform_form_settings_page_nextpay', array('GFNextPay_nextpayStd', 'nextpay_form_settings_page'));
            }
            ...
            if (rgget("page") == "gf_settings") {
                RGForms::add_settings_page(array(
                        ...
                        'handler' => array('GFNextPay_nextpayStd', 'settings_page'),
                    )
                );
            }
            ...
            if (in_array(RG_CURRENT_PAGE, array("admin-ajax.php"))) {
                add_action('wp_ajax_gf_nextpay_update_feed_active', array('GFNextPay_nextpayStd', 'update_feed_active'));
            }
        }
        if (get_option("gf_nextpay_configured")) {
            add_filter("gform_pre_render", array("GFNextPay_nextpayStd", "change_price"), 10, 1);
            add_filter("gform_confirmation", array("GFNextPay_nextpayStd", "Request"), 1000, 4);
            add_filter("gform_disable_notification", array("GFNextPay_nextpayStd", "delay_notifications"), 10, 4);
            add_filter("gform_disable_registration", array("GFNextPay_nextpayStd", "delay_registration"), 10, 4);
            add_filter("gform_disable_post_creation", array("GFNextPay_nextpayStd", "delay_posts"), 10, 3);
            add_filter("gform_is_delayed_pre_process_feed", array("GFNextPay_nextpayStd", "delay_addons"), 10, 4);

            add_action('wp', array('GFNextPay_nextpayStd', 'Verify'), 5);
        }

        add_filter("gform_logging_supported", array("GFNextPay_nextpayStd", "set_logging_supported"));

        // --------------------------------------------------------------------------------------------
        add_filter('gf_payment_gateways', array("GFNextPay_nextpayStd", 'gravityformsnextpay'), 2);
    ...
    public static function admin_notice_persian_gf()
    {
        ...
        $message = sprintf(__("برای استفاده از درگاه های پرداخت گرویتی فرم نصب بسته فارسی ساز الزامی است . برای نصب فارسی ساز %sکلیک کنید%s.", "gravityformsnextpay"), '<a href="' . admin_url("plugin-install.php?tab=plugin-information&plugin=persian-gravity-forms&TB_iframe=true&width=772&height=884") . '">', '</a>');
    ...
    {
        $class = 'notice notice-error';
        $message = sprintf(__("درگاه نکست پی نیاز به گرویتی فرم نسخه %s به بالا دارد . برای بروز رسانی هسته گرویتی فرم به %sسایت گرویتی فرم فارسی%s مراجعه نمایید .", "gravityformsnextpay"), self::$min_gravityforms_version, "<a href='http://gravityforms.ir/11378' target='_blank'>", "</a>");
        printf('<div class="%1$s"><p>%2$s</p></div>', $class, $message);
    }
    ...
    public static function add_permissions()
    {
        ...
        foreach ((array)$editable_roles as $role => $details) {
            if ($role == 'administrator' || in_array('gravityforms_edit_forms', $details['capabilities'])) {
                $wp_roles->add_cap($role, 'gravityforms_nextpay');
                $wp_roles->add_cap($role, 'gravityforms_nextpay_uninstall');
            }
        }
    }
    ...
    {
        $permission = "gravityforms_nextpay";
        if (!empty($permission))
            $menus[] = array("name" => "gf_nextpay", "label" => __("نکست پی", "gravityformsnextpay"), "callback" => array("GFNextPay_nextpayStd", "nextpay_page"), "permission" => $permission);
        return $menus;
    }
    ...
    // ------------------------NextPay.ir-------------------------
    protected static function has_access($required_permission = 'gravityforms_nextpay')
    {
        ...
        <h3>
			<span><i class="fa fa-credit-card"></i> <?php esc_html_e('نکست پی', 'gravityformsnextpay') ?>
                <a id="add-new-confirmation" class="add-new-h2"
                   href="<?php echo esc_url(admin_url('admin.php?page=gf_nextpay&view=edit&fid=' . absint(rgget("id")))) ?>"><?php esc_html_e('افزودن فید جدید', 'gravityformsnextpay') ?></a></span>
            <a class="add-new-h2"
               href="admin.php?page=gf_nextpay&view=stats&id=<?php echo absint(rgget("id")) ?>"><?php _e("نمودار ها", "gravityformsnextpay") ?></a>
        </h3>
    ...
    public static function get_config_by_entry($entry)
    {
        ...
        $feed = !empty($feed_id) ? GFNextPayData::get_feed($feed_id) : '';
    ...
    public static function get_active_config($form)
    {
        ...
        $configs = GFNextPayData::get_feed_by_form($form["id"], true);
    ...
    private static function list_page($arg)
    {
        ...
        if (!self::is_gravityforms_supported()) {
            die(sprintf(__("درگاه نکست پی نیاز به گرویتی فرم نسخه %s دارد . برای بروز رسانی هسته گرویتی فرم به %sسایت گرویتی فرم فارسی%s مراجعه نمایید .", "gravityformsnextpay"), self::$min_gravityforms_version, "<a href='http://gravityforms.ir/11378' target='_blank'>", "</a>"));
        }

        if (rgpost('action') == "delete") {
            check_admin_referer("list_action", "gf_nextpay_list");
            $id = absint(rgpost("action_argument"));
            GFNextPayData::delete_feed($id);
            ?>
            ...
            if (is_array($selected_feeds)) {
                foreach ($selected_feeds as $feed_id)
                    GFNextPayData::delete_feed($feed_id);
            }
        ...
        <div class="wrap">
            ...
            <?php if ($arg != 'per-form') { ?>
                ...
                <h2>
                    ...
                    if (get_option("gf_nextpay_configured")) { ?>
                        <a class="add-new-h2"
                           href="admin.php?page=gf_nextpay&view=edit"><?php _e("افزودن جدید", "gravityformsnextpay") ?></a>
                        <?php
                    } ?>
                </h2>
            ...
            <form id="confirmation_list_form" method="post">
                ...
                <div class="tablenav">
                    <div class="alignleft actions" style="padding:8px 0 7px 0;">
                        <label class="hidden"
                               for="bulk_action"><?php _e("اقدام دسته جمعی", "gravityformsnextpay") ?></label>
                        ...
                        <a class="button button-primary"
                           href="admin.php?page=gf_settings&subview=gf_nextpay"><?php _e('تنظیمات حساب نکست پی', 'gravityformsnextpay') ?></a>
                    </div>
                </div>
                <table class="wp-list-table widefat fixed striped toplevel_page_gf_edit_forms" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <th scope="col" id="active" class="manage-column"
                            style="width:<?php echo $arg != 'per-form' ? '50px' : '20px' ?>"><?php echo $arg != 'per-form' ? __('وضعیت', 'gravityformsnextpay') : '' ?></th>
                        <th scope="col" class="manage-column"
                            style="width:<?php echo $arg != 'per-form' ? '65px' : '30%' ?>"><?php _e(" آیدی فید", "gravityformsnextpay") ?></th>
                        <?php if ($arg != 'per-form') { ?>
                            <th scope="col"
                                class="manage-column"><?php _e("فرم متصل به درگاه", "gravityformsnextpay") ?></th>
                        <?php } ?>
                    ...
                    <tr>
                        ...
                        <th scope="col" id="active"
                            class="manage-column"><?php echo $arg != 'per-form' ? __('وضعیت', 'gravityformsnextpay') : '' ?></th>
                    ...
                    if ($arg != 'per-form')
                        $settings = GFNextPayData::get_feeds();
                    else
                        $settings = GFNextPayData::get_feed_by_form(rgget('id'), false);
                        ...
                        foreach ($settings as $setting) {
                            ...
                            <tr class='author-self status-inherit' valign="top">
                                ...
                                <td><img style="cursor:pointer;width:25px"
                                         ...
                                         alt="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformsnextpay") : __("درگاه غیر فعال است", "gravityformsnextpay"); ?>"
                                         title="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformsnextpay") : __("درگاه غیر فعال است", "gravityformsnextpay"); ?>"
                                ...
                                <td><?php echo $setting["id"] ?>
                                    <?php if ($arg == 'per-form') { ?>
                                        <div class="row-actions">
                                                <span class="edit">
                                                    <a title="<?php _e("ویرایش فید", "gravityformsnextpay") ?>"
                                                       href="admin.php?page=gf_nextpay&view=edit&id=<?php echo $setting["id"] ?>"><?php _e("ویرایش فید", "gravityformsnextpay") ?></a>
                                                    |
                                                </span>
                                                <span class="trash">
                                                    <a title="<?php _e("حذف", "gravityformsnextpay") ?>"
                                                       href="javascript: if(confirm('<?php _e("فید حذف شود؟ ", "gravityformsnextpay") ?> <?php _e("\'Cancel\' برای انصراف, \'OK\' برای حذف کردن.", "gravityformsnextpay") ?>')){ DeleteSetting(<?php echo $setting["id"] ?>);}"><?php _e("حذف", "gravityformsnextpay") ?></a>
                                                </span>
                                        </div>
                                    <?php } ?>
                                </td>
                                ...
                                <?php if ($arg != 'per-form') { ?>
                                    <td class="column-title">
                                        <strong><a class="row-title"
                                                   href="admin.php?page=gf_nextpay&view=edit&id=<?php echo $setting["id"] ?>"
                                                   title="<?php _e("تنظیم مجدد درگاه", "gravityformsnextpay") ?>"><?php echo $setting["form_title"] ?></a></strong>
                                        ...
                                        <div class="row-actions">
                                            ...
                                            <span class="trash">
                                                <a title="<?php _e("حذف فید", "gravityformsnextpay") ?>"
                                                   href="javascript: if(confirm('<?php _e("فید حذف شود؟ ", "gravityformsnextpay") ?> <?php _e("\'Cancel\' برای انصراف, \'OK\' برای حذف کردن.", "gravityformsnextpay") ?>')){ DeleteSetting(<?php echo $setting["id"] ?>);}"><?php _e("حذف", "gravityformsnextpay") ?></a>
                                                |
                                            </span>
											<span class="view">
                                                <a title="<?php _e("ویرایش فرم", "gravityformsnextpay") ?>"
                                                   href="admin.php?page=gf_edit_forms&id=<?php echo $setting["form_id"] ?>"><?php _e("ویرایش فرم", "gravityformsnextpay") ?></a>
                                                |
                                            </span>
                                            <span class="view">
                                                <a title="<?php _e("مشاهده صندوق ورودی", "gravityformsnextpay") ?>"
                                                   href="admin.php?page=gf_entries&view=entries&id=<?php echo $setting["form_id"] ?>"><?php _e("صندوق ورودی", "gravityformsnextpay") ?></a>
                                                |
                                            </span>
                                            <span class="view">
                                                <a title="<?php _e("نمودارهای فرم", "gravityformsnextpay") ?>"
                                                   href="admin.php?page=gf_nextpay&view=stats&id=<?php echo $setting["form_id"] ?>"><?php _e("نمودارهای فرم", "gravityformsnextpay") ?></a>
                                            </span>
                                        </div>
                                    </td>
                                <?php } ?>
        ...
        <script type="text/javascript">
            ...
            function ToggleActive(img, feed_id) {
                ...
                mysack.onError = function () {
                    alert('<?php _e("خطای Ajax رخ داده است", "gravityformsnextpay") ?>')
                };
                mysack.runAJAX();
                return true;
            }
    ...
    public static function update_feed_active()
    {
        ...
        $feed = GFNextPayData::get_feed($id);
        GFNextPayData::update_feed($id, $feed["form_id"], $_POST["is_active"], $feed["meta"]);
    }
                ...
                <div id="edit_payment_status_details" style="display:block">
                    <table>
                        ...
                        <tr>
                            ...
                            <td><input type="text" id="nextpay_transaction_id" name="nextpay_transaction_id"
    ...
    public static function settings_page()
    {
        ...
        if (rgpost("uninstall")) {
            ...
            update_option("gf_nextpay_settings", array_map('sanitize_text_field', $settings));
            if (isset($_POST["gf_nextpay_configured"]))
                update_option("gf_nextpay_configured", sanitize_text_field($_POST["gf_nextpay_configured"]));
            else
                delete_option("gf_nextpay_configured");
        } else {
        ...
        if (!empty($_POST)) {
            ...
            if ( isset($_POST["gf_nextpay_configured"]) && ($Response = self::Request('valid_checker', '', '', '') ) && $Response != false) {
                ...
                if ($Response === true) {
                    ...
                    echo '<div class="updated fade" style="padding:6px">' . __("در حالت تستی نیاز به ورود اطلاعات صحیح نمی باشد .", "gravityformsnextpay") . '</div>';
                } else {
		...
		else if ( isset($_GET['subview']) && $_GET['subview'] == 'gf_nextpay' && isset($_GET['updated']) ){
			echo '<div class="updated fade" style="padding:6px">' . __("تنظیمات ذخیره شدند .", "gravityformsnextpay") . '</div>';
		}
        ?>
        ...
        <form action="" method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label
                            for="gf_nextpay_configured"><?php _e("فعالسازی", "gravityformsnextpay"); ?></label></th>
                    <td>
                        <input type="checkbox" name="gf_nextpay_configured"
                               id="gf_nextpay_configured" <?php echo get_option("gf_nextpay_configured") ? "checked='checked'" : "" ?>/>
                        <label class="inline"
                               for="gf_nextpay_configured"><?php _e("بله", "gravityformsnextpay"); ?></label>
                    </td>
                </tr>

                <tr>
                    <th scope="row"><label
                            for="gf_nextpay_api_key"><?php _e("کلید مجوزدهی", "gravityformsnextpay"); ?></label></th>
                    <td>
                        <input style="width:350px;text-align:left;direction:ltr !important" type="text"
                               id="gf_nextpay_api_key" name="gf_nextpay_api_key"
                ...
                <tr>
                    ...
                    <td>
                        <input style="width:350px;" type="text" id="gf_nextpay_gname" name="gf_nextpay_gname"
                ...
                <tr>
                    <td colspan="2"><input style="font-family:tahoma !important;" type="submit"
                                           name="gf_nextpay_submit" class="button-primary"
                                           value="<?php _e("ذخیره تنظیمات", "gravityformsnextpay") ?>"/></td>
                </tr>
        ...
        <form action="" method="post">
            ...
            if (self::has_access("gravityforms_nextpay_uninstall")) {
                ...
                <div class="delete-alert alert_red">
                    ...
                    <div
                        class="gf_delete_notice"><?php _e("تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به نکست پی حذف خواهد شد", "gravityformsnextpay") ?></div>
    ...
    <?php } ?>
        ...
        <div class="wrap gforms_edit_form gf_browser_gecko">
            ...
            $config = empty($id) ? array("meta" => array(), "is_active" => true) : GFNextPayData::get_feed($id);
            $get_feeds = GFNextPayData::get_feeds();
            $form_name = '';
            ...
            <h2 class="gf_admin_page_title"><?php _e("پیکربندی درگاه نکست پی", "gravityformsnextpay") ?>
                ...
                <?php if (!empty($_get_form_id)) { ?>
					...
					<span
                        class="gf_admin_page_formid"><?php echo sprintf(__("فید: %s", "gravityformsnextpay"), $id) ?></span>
					<span
                        class="gf_admin_page_formname"><?php echo sprintf(__("فرم: %s", "gravityformsnextpay"), $form_name) ?></span>
				</span>
                <?php } ?>

            </h2>
            <a class="button add-new-h2" href="admin.php?page=gf_settings&subview=gf_nextpay"
               style="margin:8px 9px;"><?php _e("تنظیمات حساب نکست پی", "gravityformsnextpay") ?></a>

            <?php
            if (!rgempty("gf_nextpay_submit")) {
                // ------------------
                check_admin_referer("update", "gf_nextpay_feed");
                ...
                $id = GFNextPayData::update_feed($id, $config["form_id"], $config["is_active"], $config["meta"]);
                if (!headers_sent()) {
                ...
                <div class="updated fade"
                     style="padding:6px"><?php echo sprintf(__("فید به روز شد . %sبازگشت به لیست%s.", "gravityformsnextpay"), "<a href='?page=gf_nextpay'>", "</a>") ?></div>
                <?php
            }
                            ...
                            if (RG_CURRENT_VIEW != 'entry') { ?>
                                <select name="form_switcher" id="form_switcher"
                                    ...
                                    <?php foreach ($feeds as $feed) {
                                        ...
                                        <option
                                            value="<?php echo $feed["id"] ?>" <?php echo $selected ?> ><?php echo sprintf(__('فرم: %s (فید: %s)', 'gravityformsnextpay'), $feed["form_title"], $feed["id"]) ?></option>
                                    <?php } ?>
                                </select>
                                <?php
                            }
                    ...
                    <div class="gform_tab_content" id="tab_<?php echo !empty($current_tab) ? $current_tab : '' ?>">
                        <div id="form_settings" class="gform_panel gform_panel_form_settings">
                            ...
                            <form method="post" action="" id="gform_form_settings">
                                ...
                                <?php wp_nonce_field("update", "gf_nextpay_feed") ?>
                                ...
                                <table class="form-table gforms_form_settings" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select id="gf_nextpay_form" name="gf_nextpay_form"
                                                    onchange="GF_SwitchFid(jQuery(this).val());">
                                                <option
                                                    value=""><?php _e("یک فرم انتخاب نمایید", "gravityformsnextpay"); ?> </option>
                                                <?php
                                                $available_forms = GFNextPayData::get_available_forms();
                                            ...
                                            <img
                                                ...
                                                id="nextpay_wait" style="display: none;"/>
                                        </td>
                                    </tr>
                                ...
                                <?php if (empty($has_product) || !$has_product) { ?>
                                    <div id="gf_nextpay_invalid_product_form" class="gf_nextpay_invalid_form"
                                ...
                                <?php } else { ?>
                                    <table class="form-table gforms_form_settings"
                                           id="nextpay_field_group" <?php echo empty($_get_form_id) ? "style='display:none;'" : "" ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="checkbox" name="gf_nextpay_type"
                                                       id="gf_nextpay_type_subscription"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" name="gf_nextpay_desc_pm" id="gf_nextpay_desc_pm"
                                                       class="fieldwidth-1"
                                                       value="<?php echo rgar($config["meta"], "desc_pm") ?>"/>
                                                <span
                                                    class="description"><?php _e("شورت کد ها : {form_id} , {form_title} , {entry_id}", "gravityformsnextpay"); ?></span>
                                            </td>
                                        </tr>
                                        ...
                                        <tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
                                            ...
                                            <td>
                                                <select id="gf_nextpay_update_action1"
                                                        name="gf_nextpay_update_action1">
                                                    <option
                                                        value="default" <?php echo rgar($config["meta"], "update_post_action1") == "default" ? "selected='selected'" : "" ?>><?php _e("وضعیت پیشفرض فرم", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="publish" <?php echo rgar($config["meta"], "update_post_action1") == "publish" ? "selected='selected'" : "" ?>><?php _e("منشتر شده", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="draft" <?php echo rgar($config["meta"], "update_post_action1") == "draft" ? "selected='selected'" : "" ?>><?php _e("پیشنویس", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="pending" <?php echo rgar($config["meta"], "update_post_action1") == "pending" ? "selected='selected'" : "" ?>><?php _e("در انتظار بررسی", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="private" <?php echo rgar($config["meta"], "update_post_action1") == "private" ? "selected='selected'" : "" ?>><?php _e("خصوصی", "gravityformsnextpay") ?></option>
                                                </select>
                                            </td>
                                        </tr>
                                        ...
                                        <tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
                                            ...
                                            <td>
                                                <select id="gf_nextpay_update_action2"
                                                        name="gf_nextpay_update_action2">
                                                    <option
                                                        value="dont" <?php echo rgar($config["meta"], "update_post_action2") == "dont" ? "selected='selected'" : "" ?>><?php _e("عدم ایجاد پست", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="default" <?php echo rgar($config["meta"], "update_post_action2") == "default" ? "selected='selected'" : "" ?>><?php _e("وضعیت پیشفرض فرم", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="publish" <?php echo rgar($config["meta"], "update_post_action2") == "publish" ? "selected='selected'" : "" ?>><?php _e("منشتر شده", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="draft" <?php echo rgar($config["meta"], "update_post_action2") == "draft" ? "selected='selected'" : "" ?>><?php _e("پیشنویس", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="pending" <?php echo rgar($config["meta"], "update_post_action2") == "pending" ? "selected='selected'" : "" ?>><?php _e("در انتظار بررسی", "gravityformsnextpay") ?></option>
                                                    <option
                                                        value="private" <?php echo rgar($config["meta"], "update_post_action2") == "private" ? "selected='selected'" : "" ?>><?php _e("خصوصی", "gravityformsnextpay") ?></option>
                                                </select>
                                            </td>
                                        </tr>
                                            ...
                                            <td>
                                                ...
                                                foreach ($confirmations as $confirmation) { ?>
                                                    <li class="gf_nextpay_confirmation">
                                                        <input id="gf_nextpay_conf_1_<?php echo $confirmation['id'] ?>"
                                                               name="gf_nextpay_conf_1[]" type="checkbox"
                                                        ...
                                                        <label
                                                            for="gf_nextpay_conf_1_<?php echo $confirmation['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_confirmations"><?php echo $confirmation['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                        ...
                                        <tr id="gf_nextpay_confirmations_2" <?php echo !isset($form["confirmations"]) ? "style='display:none;'" : "" ?>>
                                            ...
                                            <td>
                                                ...
                                                foreach ($confirmations as $confirmation) { ?>
                                                    <li class="gf_nextpay_confirmation">
                                                        <input id="gf_nextpay_conf_2_<?php echo $confirmation['id'] ?>"
                                                               name="gf_nextpay_conf_2[]" type="checkbox"
                                                               class="confirmation_checkbox"
                                                               value="<?php echo $confirmation['id'] ?>" <?php checked(true, in_array($confirmation['id'], $selected_confirmations)) ?> />
                                                        <label
                                                            for="gf_nextpay_conf_2_<?php echo $confirmation['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_confirmations"><?php echo $confirmation['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                        ...
                                        <tr id="gf_nextpay_confirmations_3" <?php echo !isset($form["confirmations"]) ? "style='display:none;'" : "" ?>>
                                            ...
                                            <td>
                                                ...
                                                foreach ($confirmations as $confirmation) { ?>
                                                    <li class="gf_nextpay_confirmation">
                                                        <input id="gf_nextpay_conf_3_<?php echo $confirmation['id'] ?>"
                                                               name="gf_nextpay_conf_3[]" type="checkbox"
                                                               class="confirmation_checkbox"
                                                               value="<?php echo $confirmation['id'] ?>" <?php checked(true, in_array($confirmation['id'], $selected_confirmations)) ?> />
                                                        <label
                                                            for="gf_nextpay_conf_3_<?php echo $confirmation['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_confirmations"><?php echo $confirmation['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                            ...
                                            <td>
                                                ...
                                                foreach ($notifications as $notification) { ?>
                                                    <li class="gf_nextpay_notification">
                                                        <input
                                                            id="gf_nextpay_notif_1_<?php echo $notification['id'] ?>"
                                                            name="gf_nextpay_notif_1[]" type="checkbox"
                                                        ...
                                                        <label
                                                            for="gf_nextpay_notif_1_<?php echo $notification['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_notifications"><?php echo $notification['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                        ...
                                        <tr id="gf_nextpay_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
                                            ...
                                            <td>
                                                ...
                                                foreach ($notifications as $notification) { ?>
                                                    <li class="gf_nextpay_notification">
                                                        <input
                                                            id="gf_nextpay_notif_2_<?php echo $notification['id'] ?>"
                                                            name="gf_nextpay_notif_2[]" type="checkbox"
                                                            class="notification_checkbox"
                                                            value="<?php echo $notification['id'] ?>" <?php checked(true, in_array($notification['id'], $selected_notifications)) ?> />
                                                        <label
                                                            for="gf_nextpay_notif_2_<?php echo $notification['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_notifications"><?php echo $notification['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                        ...
                                        <tr id="gf_nextpay_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
                                            ...
                                            <td>
                                                ...
                                                foreach ($notifications as $notification) { ?>
                                                    <li class="gf_nextpay_notification">
                                                        <input
                                                            id="gf_nextpay_notif_3_<?php echo $notification['id'] ?>"
                                                            name="gf_nextpay_notif_3[]" type="checkbox"
                                                            class="notification_checkbox"
                                                            value="<?php echo $notification['id'] ?>" <?php checked(true, in_array($notification['id'], $selected_notifications)) ?> />
                                                        <label
                                                            for="gf_nextpay_notif_3_<?php echo $notification['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_notifications"><?php echo $notification['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                        ...
                                        <tr id="gf_nextpay_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
                                            ...
                                            <td>
                                                ...
                                                foreach ($notifications as $notification) { ?>
                                                    <li class="gf_nextpay_notification">
                                                        <input
                                                            id="gf_nextpay_notif_4_<?php echo $notification['id'] ?>"
                                                            name="gf_nextpay_notif_4[]" type="checkbox"
                                                            class="notification_checkbox"
                                                            value="<?php echo $notification['id'] ?>" <?php checked(true, in_array($notification['id'], $selected_notifications)) ?> />
                                                        <label
                                                            for="gf_nextpay_notif_4_<?php echo $notification['id'] ?>"
                                                            class="inline"
                                                            for="gf_nextpay_selected_notifications"><?php echo $notification['name']; ?></label>
                                                    </li>
                                                <?php } ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="checkbox" name="gf_nextpay_addon"
                                                       id="gf_nextpay_addon_true"
                                                       value="true" <?php echo rgar($config['meta'], 'addon') == "true" ? "checked='checked'" : "" ?>/>
                                                <label for="gf_nextpay_addon"></label>
                                                <span
                                                    class="description"><?php _e('گرویتی فرم دارای ادان های مختلف وابسته به GFAddon نظیر ایمیل مارکتینگ و ... می باشد که دارای متد add_delayed_payment_support هستند. در صورتی که میخواهید این ادان ها تنها در صورت تراکنش موفق عمل کنند این گزینه را تیک بزنید.', 'gravityformsnextpay'); ?></span>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="radio" name="gf_nextpay_shaparak"
                                                       id="gf_nextpay_shaparak_raygan"
                                                       value="raygan" <?php echo rgar($config['meta'], 'shaparak') != "sadt" ? "checked" : "" ?>/>
                                                <label class="inline"
                                                       for="gf_nextpay_shaparak_raygan"><?php _e("رایگان شود", "gravityformsnextpay"); ?></label>

                                                <input type="radio" name="gf_nextpay_shaparak"
                                                       id="gf_nextpay_shaparak_sadt"
                                                       value="sadt" <?php echo rgar($config['meta'], 'shaparak') == "sadt" ? "checked" : "" ?>/>
                                                <label class="inline"
                                                       for="gf_nextpay_shaparak_sadt"><?php echo $minprice . __(" شود ", "gravityformsnextpay"); ?></label>
                                                <span
                                                    class="description"><?php _e('(قابل استفاده برای زمانیکه منطق شرطی درگاه فعال نباشد)', 'gravityformsnextpay'); ?></span>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="checkbox" id="gf_nextpay_conditional_enabled"
                                                       name="gf_nextpay_conditional_enabled" value="1"
                                                       onclick="if(this.checked){jQuery('#gf_nextpay_conditional_container').fadeIn('fast');} else{ jQuery('#gf_nextpay_conditional_container').fadeOut('fast'); }" <?php echo rgar($config['meta'], 'nextpay_conditional_enabled') ? "checked='checked'" : "" ?>/>
                                                <label
                                                    for="gf_nextpay_conditional_enable"><?php _e("فعالسازی این درگاه اگر شرط زیر برقرار باشد", "gravityformsnextpay"); ?></label><br/>

                                                <table cellspacing="0" cellpadding="0">
                                                    <tr>
                                                        <td>
                                                            <div
                                                                id="gf_nextpay_conditional_container" <?php echo !rgar($config['meta'], 'nextpay_conditional_enabled') ? "style='display:none'" : "" ?>>
                                                                <div id="gf_nextpay_conditional_fields"
                                                                     style="display:none">
                                                                    <select id="gf_nextpay_conditional_field_id"
                                                                            name="gf_nextpay_conditional_field_id"
                                                                            class="optin_select"
                                                                            onchange='jQuery("#gf_nextpay_conditional_value_container").html(GetFieldValues(jQuery(this).val(), "", 20));'></select>
                                                                    <select id="gf_nextpay_conditional_operator"
                                                                            name="gf_nextpay_conditional_operator">
                                                                        <option
                                                                            value="is" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "is" ? "selected='selected'" : "" ?>><?php _e("هست", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value="isnot" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "isnot" ? "selected='selected'" : "" ?>><?php _e("نیست", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value=">" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == ">" ? "selected='selected'" : "" ?>><?php _e("بزرگ تر است از", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value="<" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "<" ? "selected='selected'" : "" ?>><?php _e("کوچک تر است از", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value="contains" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "contains" ? "selected='selected'" : "" ?>><?php _e("شامل میشود ", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value="starts_with" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "starts_with" ? "selected='selected'" : "" ?>><?php _e("شروع میشود با", "gravityformsnextpay") ?></option>
                                                                        <option
                                                                            value="ends_with" <?php echo rgar($config['meta'], 'nextpay_conditional_operator') == "ends_with" ? "selected='selected'" : "" ?>><?php _e("تمام میشود با", "gravityformsnextpay") ?></option>
                                                                    </select>
                                                                    <div id="gf_nextpay_conditional_value_container"
                                                                         name="gf_nextpay_conditional_value_container"
                                                                         style="display:inline;"></div>
                                                                </div>
                                                                <div id="gf_nextpay_conditional_message"
                                        ...
                                        <tr>
                                            <td>
                                                <input type="submit" class="button-primary gfbutton"
                                                       name="gf_nextpay_submit"
                                                       value="<?php _e("ذخیره", "gravityformsnextpay"); ?>"/>
                                            </td>
                                        </tr>
                                        </tbody>
                                    </table>
    ...
    public static function Request($confirmation, $form, $entry, $ajax)
    {
        ...
        if (!$valid_checker) {
            ...
            $Description = __('جهت بررسی تنظیمات صحیح بودن تنظیمات درگاه گرویتی فرم نکست پی', 'gravityformsnextpay');
